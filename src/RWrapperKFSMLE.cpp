...
double objective_function(unsigned n, const double* x, double* grad, void* data)
{
    ...
    double ll = LL(parameters, data_in->X, data_in->tau, data_in->m, data_in->d);
...
Eigen::MatrixXd ComputeHessian(const Eigen::VectorXd& parameters, void* data, double h) {
    ...
    for (int i = 0; i < parameters.size(); ++i) {
        ...
        for (int j = i; j < parameters.size(); ++j) {
            ...
            double f1 = LL(parameters_plus, data_in->X, data_in->tau, data_in->m, data_in->d);
            double f2 = LL(parameters, data_in->X, data_in->tau, data_in->m, data_in->d);
            double f3 = LL(parameters_minus, data_in->X, data_in->tau, data_in->m, data_in->d);
