...
namespace LogLike {
	...
	*/
	inline double MVLL(
		...
		const double& initial_kf_var_cov
	) {
		...
		for (unsigned m = 0; m < M; ++m) {
			...
			for (unsigned i = 0; i < lower_chol_factor.rows(); i++) {
		...
		Eigen::MatrixXd state_var_cov = initial_kf_var_cov * Eigen::MatrixXd::Identity(d, d);
		...
		for (unsigned t = 1; t < T; ++t) {
			...
			if (day_indicator(t) == 0.0) {

				// Re-initialise the KFS if it is the first day of the date change
				if (day_indicator(t) != day_indicator(t - 1)) {
					state_mean = Eigen::VectorXd::Zero(d);
					state_var_cov = initial_kf_var_cov * Eigen::MatrixXd::Identity(d, d);
				}
				continue;
			}
