...
#' @param m Integer, the first dimension parameter.
#' @param d Integer, the second dimension parameter.
...
NLoptKFS <- function(start, X, tau, m, d, xtol = 10e-5, stop_val = 10e-10, algorithm = "LN_NELDERMEAD", hessian = TRUE, step_size = 10e-5, verbose = FALSE) {

  # Convert inputs to matrices

  errorOccurred <- FALSE

  startR <- try(as.matrix(start), silent = TRUE)
  if (inherits(startR, "try-error")) {
    errorOccurred <- TRUE
  }

  XR <- if (!errorOccurred) try(as.matrix(X), silent = TRUE) else NULL
  ...
  .Call('_KFSMLE_FastOptim', PACKAGE = 'KFSMLE', startR, XR, tauR, m, d, xtol, stop_val, algorithm_id, hessian, step_size, verbose)
}
